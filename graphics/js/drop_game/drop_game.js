...
class ParachuteDropper extends SpriteContainer {
  ...
  transmitDropStatus(onTarget, winner, voluntary) {
    nodecg.sendMessage('drop-game-drop-result', {
      'onTarget': onTarget,
      'winner': winner,
...
nodecg.listenFor('drop-game-drop', dropInfo => {
  engine.drop(dropInfo.name, dropInfo.emoteId);
});

nodecg.listenFor('drop-game-cut', name => {
  engine.cut(name);
});

nodecg.listenFor('drop-game-abdicate', name => {
